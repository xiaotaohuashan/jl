...
public class RingView extends View {
    ...
    private List<RingBean> rateList;
...
//        Log.e("内部圆点:", dip2px(circleCenterX + mPaintWidth * 2 + leftMargin) + " --- " + dip2px(circleCenterY + mPaintWidth * 2 + topMargin));
    ...
    private void drawArcCenterPoint(Canvas canvas, int position) {
        ...
        if (preRate / 2 + rateList.get(position).getRate() / 2 < 5) {
            extendLineWidth += 20;
            rate -= 0.05f;
        } else {
            extendLineWidth = 20;
        ...
        mPaint.setStyle(Paint.Style.STROKE);
        canvas.drawText(rateList.get(position).getName() + rateList.get(position).getRate() + "%", floats[6], floats[7] + dip2px(showRateSize) / 3, mPaint);
        preRate = rateList.get(position).getRate();
    }
...
//        canvas.drawCircle(circleCenterX, circleCenterY, ringInnerRidus, mPaint);
        if (rateList != null) {
            endAngle = getAngle(rateList.get(position).getRate());
        }
//        Log.e("preAngle:", "" + preAngle + "   endAngle:" + endAngle);
        canvas.drawArc(rectF, preAngle, endAngle-1, true, mPaint);
